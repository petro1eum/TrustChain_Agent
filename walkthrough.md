...
### Enterprise Feature: Durable Task Engine & DLQ (At-Least-Once Delivery)
Помимо инъекций, массовая отправка вебхуков (например, пиковая загрузка на 'черную пятницу' или DDoS) может вызвать выгорание токенов и падение сервера. Для решения этой проблемы и гарантии 100% доставки реализован **Durable Task Engine** на базе SQLite:
- В `triggers.py` внедрена транзакционная запись: эндпоинт **моментально пишет Payload в локальную базу `tasks.db` со статусом `PENDING`** и сразу отвечает `202 Accepted` (чтобы внешняя система, например Salesforce, не падала по таймауту).
- Независимый асинхронный воркер (`queue_worker.py`) непрерывно опрашивает БД. Он блокирует задачу, спавнит агента и по итогу переводит статус в `SUCCESS`. 
- **OOM / Crash Resilience:** Если сервер будет жестко перезагружен (например, убит Docker-контейнер OOM-киллером), все `PENDING` и `RUNNING` задачи останутся в SQLite. При следующем старте `uvicorn`, воркер продолжит с того же места, ни один оплаченный вебхук не будет потерян.
