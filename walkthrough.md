...
### Enterprise Feature: Asynchronous Webhook Queue (Token Exhaustion Defense)
...
- В `triggers.py` внедрена элегантная **Асинхронная in-memory очередь** на основе `asyncio.Semaphore`.
- Когда поступает писк вебхуков, эндпоинт **моментально отвечает `202 Accepted`** (чтобы Salesforce не падал по таймауту) и аккуратно складывает задачи в очередь (`_pending_webhook_count`).
- Воркеры (`MAX_CONCURRENT_WEBHOOKS = 10`) последовательно разбирают задания из очереди, гарантируя ровную нагрузку на LLM (защита от скачков биллинга).
- Как абсолютная "Линия Маннергейма" добавлен жесткий хард-лимит очереди (`MAX_QUEUE_SIZE = 1000`). Только если память сервера полностью забита этой очередью, новые вебхуки начнут отбиваться статусом `HTTP 429 Too Many Requests`.
