...
const DEMO_ARTIFACTS: Record<string, Artifact> = {
    'art-security': {
        ...
        content: `# Security Analysis — Commit a7f3b2c
...
---
*Generated by TrustChain Agent v0.3.0 — Ed25519 signed*`,
...
async function alertSecurityTeam(entry: any): Promise<void> {
  ...
  console.log(\`🚨 Violation: \${JSON.stringify(entry)}\`);
}`,
...
- Node.js ≥20 for Ed25519 native support`,
    ...
    'art-graph': {
        ...
        tier: 'pro' as Tier,
        content: `# Execution Graph — Session a7f3b2c
...
## DAG Visualization
\`\`\`
git_diff [a7f3b2c] ──→ security_scan [c9d0e1f] ──→ verify_chain [b5a6c7d]
                                                       │
                                                       └──→ code_generate [a1b2c3d]
                                                                │
compiance_evaluate [e1f2a3b] ──→ export_pdf [f5a6b7c]
\`\`\`

## Fork Analysis
No forks detected — all execution paths are linear. This indicates deterministic agent behavior with no branching decisions.
...
---
*Generated by ExecutionGraph (TrustChain Pro) — DAG Forensics Module*`,
    ...
    'art-analytics': {
        ...
        tier: 'pro' as Tier,
        content: `# TrustChain Analytics — Live Snapshot
...
---
*Powered by TrustChainAnalytics (Pro) — Real-time signing analytics*`,
...
const DEMO_MESSAGES: Message[] = [
    ...
    },
    {
        id: 'm2',
        role: 'assistant',
        content: `Audit complete. Scanned **14 files**, found **1 CVE** (lodash@4.17.20 → CVE-2021-23337, Medium). All 8 SOC 2 controls passed. Chain integrity verified across all 7 operations — no forks, replays, or orphans detected.`,
        ...
        executionSteps: [
            {
                ...
                detail: 'Plan: scan code → audit deps → verify chain → build graph → compliance report',
                latencyMs: 45,
            },
            {
...
`);
    ...
    // ── Configure agentCallbacksService on mount ──
    useEffect(() => {
    ...
    const extractArtifactsFromEvents = useCallback((events: MessageEvent[]): { artifactIds: string[]; newArtifacts: Record<string, Artifact> } => {
        ...
        for (const ev of events) {
            ...
            if (ev.type === 'tool_result') {
                ...
                // Parse result — handle both string and object results
                let result: any;
                try {
                ...
                const artifactContent = result.content || args.content || args.file_text
                    || `Artifact создан: ${result.filename}\nПуть: ${result.path || '/mnt/user-data/outputs/' + result.filename}\nРазмер: ${result.size || 'N/A'}`;
    ...
    // ── Ask Agent callback (used by ToolManager & SkillsManager) ──
