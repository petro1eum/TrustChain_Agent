...
`;
  ...
  /**
   * Поиск по conversations через Backend API
   */
  async handleConversationSearch(args: any): Promise<any> {
    const norm = this.deps.normalizeArgs(args, {
      q: 'query',
      search_query: 'query',
      search_term: 'query',
    ...
    const query = norm.query || norm.q || '';
    if (!query || query.trim() === '') {
      return {
        success: false,
        error: 'query is required',
        description: 'Требуется указать поисковый запрос'
      };
    }

    const limit = Math.max(1, Math.min(Number(norm.limit || 10), 50));
    ...
    try {
      ...
      return {
        ...
        count: (data.conversations || []).length + (data.messages || []).length,
        description: `Найдено ${(data.conversations || []).length} conversations и ${(data.messages || []).length} сообщений по запросу "${query.trim()}"`
      };
    } catch (error: any) {
      return {
        success: false,
        error: error.message || 'Ошибка поиска conversations',
        query: query.trim(),
        description: 'Ошибка при поиске по истории разговоров'
      };
    }
  }

  /**
   * Получить последние conversations через Backend API
   */
  async handleRecentChats(args: any): Promise<any> {
    const norm = this.deps.normalizeArgs(args, {
      limit: 'limit',
      max_results: 'limit'
    });
    ...
    try {
      ...
      return {
        ...
        description: `Получено ${(conversations || []).length} последних conversations`
      };
    } catch (error: any) {
      return {
        success: false,
        error: error.message || 'Ошибка получения recent chats',
        description: 'Ошибка при получении последних разговоров'
      };
    }
  }

  /**
   * Match items from PDF specification to catalog products
   * Complete flow: PDF → Extract → Search → Export with mapping
   * All done in single backend call (no duplicate extraction)
   */
  async handleMatchSpecificationToCatalog(args: any): Promise<any> {
    const norm = this.deps.normalizeArgs(args, {
      file_name: 'filename',
      file: 'filename',
      page_start: 'pageStart',
    ...
    const pageEnd = Number(norm.pageEnd);
    if (!Number.isFinite(pageStart) || !Number.isFinite(pageEnd)) {
      return { success: false, error: 'page_start and page_end are required' };
    }
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/opensearch/match-specification`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          page_start: pageStart,
          page_end: pageEnd,
          search_column: norm.searchColumn || 'наименование',
          max_matches_per_item: norm.maxMatches || 3
        })
      });
      ...
      if (!response.ok) {
        ...
        return {
          ...
          description: errText.slice(0, 200) || 'Backend error'
        };
      }

      const rawResult = await response.json();

      // Unwrap TrustChain response
      const signature = rawResult.signature;
      const signature_id = rawResult.signature_id;
      const certificate = rawResult.certificate;
      ...
      if (!result.success) {
        return {
          success: false,
          error: result.error || 'matching_failed',
          description: result.description || result.detail || 'Не удалось выполнить сопоставление'
        };
      }

      // Upload Excel to artifacts
      const upload = await ArtifactsService.uploadArtifact({
        filename: result.filename,
        contentBase64: result.content_base64
      });
      ...
      return {
        ...
        description: `Сопоставление завершено: ${result.statistics?.found || 0} найдено, ${result.statistics?.not_found || 0} не найдено`,
        match_results_preview: result.match_results,
      ...
      console.error('[handleMatchSpecificationToCatalog] Error:', error);
      return {
        success: false,
        error: error.message || 'backend_call_failed',
        description: 'Не удалось вызвать backend для сопоставления'
      };
    }
  }
}
