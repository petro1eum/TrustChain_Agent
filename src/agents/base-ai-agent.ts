    ...
    if (loopMessages && loopMessages.length > 0) {
      ...
      try {
        const synthesisMessages = [
          ...
          {
            role: 'user' as const,
            content: `На основе результатов поиска выше, дай РАЗВЁРНУТЫЙ ОТВЕТ пользователю.

СТРОГИЕ ПРАВИЛА:
1. Рекомендуй ТОЛЬКО товары из результатов поиска выше — никаких других!
2. ВСЕГДА указывай JDE код (поле "Номер JDE" или article) для каждого товара
3. Бери характеристики ТОЛЬКО из данных поиска (params), не придумывай
4. Объясни ПОЧЕМУ выбранные товары подходят под запрос
5. Укажи цены из поля "Цена варианта на santech.ru без НДС"

ФОРМАТ ОТВЕТА:
- Краткий расчёт потребности (если нужен)
- 1-3 лучших варианта с JDE кодами и ценами
- Почему именно эти товары подходят
- Конкретная рекомендация

НЕ добавляй маркетинговую информацию которой нет в данных поиска!`
          }
        ];
